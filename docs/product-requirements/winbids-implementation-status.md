...
|---|---|---|---|
| Product 2 Richer Evidence / Artifact Links v1 | Done locally: pursue/no-bid reason details now include additive `evidenceRefs` linked to match snapshots, supplier profile, bid detail/source URLs, generated output, citation ids, and attachment download routes; Intent detail renders linked evidence chips while preserving the legacy evidence label fallback. | Compliance evidence mapping can now reuse the same citation/artifact vocabulary; deeper document parsing remains future AI work. | Continue with Product 2 compliance evidence mapping or Knowledge Station Lite. |
...
| Product 2 Qualification Upgrade | Match score、AI-like brief、Submission Guidance、Compliance Manifest Lite、Pursue/No-Bid Lite 已有 deterministic 版本；Qualification Evidence Citations v1、Document-Grounded Q&A v1、Amendment/Addenda Awareness v1、No-Bid Taxonomy + Qualification Risk Explanations v1、Richer Evidence / Artifact Links v1 已完成，Intent 级 citation snapshot、只读 citations API/client、freshness/refresh API/client、Pro-gated Q&A API/client、结构化 pursuit reason details、reason evidence refs、Intent detail 证据面板/证据 chip/新鲜度状态/刷新按钮/决策原因分类和基于证据问答已接入。 | compliance evidence mapping。 | 下一步做 Product 2 compliance evidence mapping 或 Knowledge Station Lite。 |
...
- The next highest Product 2 risk is compliance traceability: reason cards now link to evidence, but Compliance Manifest items still need direct evidence mapping.
...
1. **Product 2 Qualification Upgrade Continuation**
   - Add compliance evidence mapping on top of the existing citation and evidence-ref vocabulary.
