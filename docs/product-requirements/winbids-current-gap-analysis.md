...
- Python crawler execution now runs through the `fetch-task` JSON stdin/stdout contract (`crawler/apsi_crawler/cli.py`): the frontend passes `source_id`/`label`/`state_code`/`provider_family`/`fetch_config` from a `data_sources` row in the task payload, not argv or a hardcoded `Source` object. The retired `fetch-state --output-json` command, and the `source_validity` metadata it used to emit for downstream MySQL/crawler logs, no longer exist; source authority/trust status/evidence mode/validity notes still live only in `frontend/src/lib/state-crawler-sources.ts` and are not part of the task/result JSON.
