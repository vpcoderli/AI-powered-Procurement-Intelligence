...
- **解析 sidecar**: Python 3.12 + [Scrapling](https://github.com/D4Vinci/Scrapling) 0.4.15 基础包（仅解析器，不含 fetchers / 反检测组件），stdlib `http.server` 暴露 `/health`、`/extract`（详情字段）与 `/extract-list`（列表页），独立 Docker 镜像
- **浏览器 sidecar**: Python 3.12 + Playwright 1.63 / Chromium，`/download`（表单/JS 驱动的公开附件下载）与 `/render`（渲染后 HTML），登录墙 / 跨主机 / 大小 / 超时守卫，单并发
...
爬虫与 Scrapling 的完整执行链路、字段合并规则、验证结果及待补齐事项见 [全链路逻辑梳理](docs/architecture/crawler-enrichment-flow.md)。
...
│   ├── scripts/             # 迁移、seed、四个 worker（crawler / events / notifications / attachments）、质量门禁与运维脚本
...
│   │   ├── sources/         # 数据源定义与治理元数据
│   │   ├── normalizers/     # 原始数据标准化
│   │   ├── storage/         # SQLite 写入、附件归档（archive-attachments CLI：魔数校验、相对路径、浏览器下载客户端）
...
│   └── browser-downloader/  # 无头浏览器 sidecar（Playwright/Chromium，/health、/download、/render，只访问公开页面）
├── docs/                    # 产品需求、运维手册、QA 报告、设计文档
└── docker-compose.yml       # 本地 demo 容器编排（含 scrapling-extractor；workers profile 含 crawler / attachment worker 与 browser-downloader）
```
