...
考虑到 MVP 阶段的资源限制（单名开发者）与访问合规边界，架构设计遵循以下原则：
- **分层策略**：优先直接抓取（API/HTML 解析），在 JavaScript 渲染、分页或公开接口缺失时使用模拟浏览器；不处理验证码、人机验证或明确禁止自动化访问的站点。
- **模块化与隔离**：各州爬虫脚本独立，单点故障不影响全局。
- **增量更新**：支持基于时间戳的增量拉取，减少服务器压力和访问失败风险。
...
| **动态渲染** | Playwright (或 Selenium) | 应对 JavaScript 渲染、分页、公开接口缺失等场景；不处理验证码、人机验证或明确禁止自动化访问的站点。 |
...
- **访问合规与失败处理**：
  - **User-Agent 标识**：使用清晰、稳定、可识别的 User-Agent，并在需要时提供联系信息，避免伪装或规避访问限制。
  - **请求延迟 (Delay)**：遵守 `robots.txt`，在请求之间加入随机的 `sleep`（例如 1-3 秒）。
  - **受限来源处理**：若来源限制访问或连续失败，则降频、暂停该来源、记录失败并人工评估访问许可。
