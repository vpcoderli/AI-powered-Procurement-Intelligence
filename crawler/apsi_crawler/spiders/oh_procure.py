...
class OhProcureError(Exception):
    pass
...
def _record_from_row(row, source):
    source_bid_id = row.get("Opportunity ID")
    if not source_bid_id:
        raise OhProcureError("Ohio procurement row is missing opportunity id")
    ...
    return {
        ...
        "source_url": absolute_url(
            source.base_url,
            links.get("Opportunity ID") or links.get("Opportunity Title") or "",
        ),
...
def _record_from_json(record, source):
    source_bid_id = _first_present(
        record,
        ("source_bid_id", "opportunityId", "opportunity_id", "id"),
    )
    if not source_bid_id:
        raise OhProcureError("Ohio procurement record is missing source id")

    return {
        "source_bid_id": source_bid_id,
        "title": _first_present(record, ("title", "opportunityTitle", "opportunity_title")),
        "description": _first_present(
            record,
            ("description", "summary", "title", "opportunityTitle", "opportunity_title"),
        ),
        ...
        "original_category": _first_present(record, ("original_category", "category", "type")),
        "source_url": absolute_url(
            source.base_url,
...
def _records_from_payload(payload, source):
    if isinstance(payload, list):
        ...
        records = []
    if not records:
        raise OhProcureError("Ohio procurement fixture JSON did not contain opportunities")
    return [_record_from_json(record, source) for record in records]


def _records_from_html(html, source):
    try:
        rows = extract_table_rows(html, required_headers=OH_PROCURE_HEADERS)
    except HtmlPageError as error:
        raise OhProcureError("Ohio procurement page missing expected opportunity table headers") from error
    return [_record_from_row(row, source) for row in rows]


def fetch_oh_procure_opportunities(
    source,
    query=None,
    limit=25,
    session=None,
    ...
    if fixture_json:
        ...
        if fixture_html:
            ...
            try:
                ...
                raise OhProcureError(str(error)) from error
