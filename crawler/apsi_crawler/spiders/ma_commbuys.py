...
def _record_from_row(row, source):
    ...
    return {
        ...
        "title": row.get("Bid Title"),
        "description": row.get("Bid Title"),
        "issuer_name": row.get("Purchasing Organization"),
        "published_date": row.get("Published Date"),
        "deadline_date": row.get("Closing Date"),
        "original_category": row.get("Category"),
        "source_url": absolute_url(
            source.base_url,
            links.get("Bid Number") or links.get("Bid Title") or "",
        ),
...
def _records_from_html(html, source):
    try:
        rows = extract_table_rows(html, required_headers=MA_COMMBUYS_HEADERS)
    except HtmlPageError as error:
        raise MaCommbuysError("COMMBUYS page missing expected bid table headers") from error
