...
KS_ESUPPLIER_URL = "https://supplier.sok.ks.gov/psc/sokfsprdsup/SUPPLIER/ERP/c/SCP_PUBLIC_MENU_FL.SCP_PUB_BID_CMP_FL.GBL"
KS_ESUPPLIER_HEADERS = (
    "Event Name",
    "Business Unit",
    "Event ID",
    "Start Date",
...
class KsEsupplierError(Exception):
    pass
...
def _record_from_row(row):
    ...
    if not source_bid_id:
        raise KsEsupplierError("Kansas eSupplier row is missing event id")
    return {
...
def _extract_opportunity_rows(html):
    ...
    last_missing = list(KS_ESUPPLIER_HEADERS)
    ...
    for table in tables:
        ...
        for index, row in enumerate(table):
            ...
            missing = [header for header in KS_ESUPPLIER_HEADERS if header not in row_headers]
            if missing:
    ...
    raise HtmlPageError("HTML table was missing required headers: " + ", ".join(last_missing))


def fetch_ks_esupplier_opportunities(
    source,
    query=None,
    limit=25,
    session=None,
    ...
    if fixture_html:
        ...
        try:
            html = fetch_html(KS_ESUPPLIER_URL, session=session, timeout=timeout)
        except HtmlPageError as error:
            raise KsEsupplierError(str(error)) from error
